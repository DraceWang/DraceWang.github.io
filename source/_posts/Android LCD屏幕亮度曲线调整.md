...
{% note tip 推出设置中的显示界面，进度条位置就不记录了%}
A:这个主要是回调那部分没有改好。
{% endnote %}

{% note tip 恢复出厂后手动进度的位置不在原来的50%的位置。%} 
...
源码提供了`frameworks/base/core/res/res/values/config.xml`这个配置中直接更改节点值的方式来修改自动背光曲线，这种方式我们这里不做过多的讨论了。下面来说说自定义的背光曲线怎么弄。
